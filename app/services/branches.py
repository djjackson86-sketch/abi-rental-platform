...
def delete_branch(branch_id):
    ...
    # Cash-up history belongs to the depot being deleted: its cash-used and bank
    # drop off lines first (libsql autocommits, so children are deleted
    # explicitly), then the day rows themselves.
    db.execute(
        "DELETE FROM cash_used WHERE cash_up_id IN (SELECT id FROM cash_ups WHERE branch_id = ?)",
        (branch_id,),
    )
    db.execute(
        "DELETE FROM cash_bank_drops WHERE cash_up_id IN (SELECT id FROM cash_ups WHERE branch_id = ?)",
        (branch_id,),
    )

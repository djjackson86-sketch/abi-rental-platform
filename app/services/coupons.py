...
def get_coupon(coupon_id):
    return get_db().execute("SELECT * FROM coupons WHERE id = ?", (coupon_id,)).fetchone()


def _coupon_payload(form):
    ...
    try:
        ...
        raise ValueError("Coupon discount must be a number") from exc
    if value <= 0:
...
def create_coupon(form):
    payload = _coupon_payload(form)
    db = get_db()
    try:
        db.execute(
            """INSERT INTO coupons (code, description, discount_type, value, active, created_at)
            VALUES (?, ?, ?, ?, ?, ?)""",
            (payload["code"], payload["description"], payload["discount_type"], payload["value"], payload["active"], now()),
        )
        db.commit()
...
def update_coupon(coupon_id, form):
    if not get_coupon(coupon_id):
        raise ValueError("Coupon not found")
    payload = _coupon_payload(form)
    db = get_db()
    try:
        db.execute(
            """UPDATE coupons SET code = ?, description = ?, discount_type = ?, value = ?, active = ?
            WHERE id = ?""",
            (payload["code"], payload["description"], payload["discount_type"], payload["value"], payload["active"], coupon_id),
        )
        db.commit()
    except Exception as exc:
        db.rollback()

...
def test_orders_date_range_filters_metrics_and_shows_deposit_to_process(client, app):
    ...
    assert b'<small>Revenue received</small><b>R0.00</b>' in filtered.data
...
def test_invoice_line_items_exclude_vat_and_summarise_it(client, app):
    ...
    assert b'<th>Rate</th>' in invoice.data
    assert b'<th>Subtotal</th>' in invoice.data
    ...
    assert (invoice.data.index(b'<th>Product</th>')
            ...
            < invoice.data.index(b'<th>Rate</th>')
            < invoice.data.index(b'<th>Subtotal</th>')
...
def test_invoice_uses_collection_branch_issuer_and_bank_details(client, app):
    ...
    assert pdf.index(b'(Total without VAT) Tj') < pdf.index(b'(VAT) Tj') < pdf.index(b'(Total with VAT) Tj') < pdf.index(b'(Banking details) Tj')
    assert pdf.index(b'Thank you for your business.') < pdf.index(b'Banking details')
    assert b'0.08 0.39 1 rg' not in pdf
...
def test_main_can_manage_users_and_staff_have_restricted_access(client, app):
    ...
    # Tickets ABI-341953032/034 give staff their own metric bar (red Due card plus
    # the unprocessed-deposit count and value), so the restricted-access check is the
    # absence of the main-only totals row (its id and its toggle), not of the cards.
    assert b'id="orders-metrics"' not in orders.data
    assert b'orders-staff-metrics' in orders.data
    assert b'Unprocessed deposits' in orders.data
